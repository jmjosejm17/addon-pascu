...
__home__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/', ''))
...
__art__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/art', ''))
__cbx_pages__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/art/cbx', ''))
__icons__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/art/icons', ''))
__libdir__ = xbmc.translatePath(os.path.join('special://xbmc/system/players/dvdplayer/', ''))
__tools__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/resources/tools', ''))
__addons__ = xbmc.translatePath(os.path.join('special://home/addons/', ''))
__resources__ = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pascu4/resources', ''))
...
ADDON=xbmcaddon.Addon(id='plugin.video.pascu4')
INTRO = xbmc.translatePath(os.path.join('special://home/plugin.video.pascu4/intro.mp4'))
    ...
    if ADDON.getSetting('intro') == "true":
       ...
       # pass
        if ADDON.getSetting('message') == "true":
            ...
            msg = intro.ok("[COLOR blue]http://Pascu120.tk[/COLOR]", "[COLOR orange]****************[COLOR red]-------NUEVO ADOONS------[COLOR orange]****************[/COLOR]",
                            "**********************[COLOR lime]Pascu 0.1.4[/COLOR]**********************",
...
# Entry point
def run():
    plugintools.log('[%s %s] ---> pascu.run <--- ' % (addonName, addonVersion))
...
def main_list(params):
    ...
    try:
        ...
        data = plugintools.read(mastermenu)
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('pascu', "XML no reconocido...", 3 , __art__+'icon.png'))
...
def simpletv_items(params):
    ...
    if title == 'search.txt':
        ...
        data = file.readline()
        if data == "":
            ok = plugintools.message("pascu", "Sin resultados")
            return ok
    else:
        title = params.get("title")
...
def search_channel(params):
    ...
    if buscar == "":
        ...
        cat = ""
        if texto == "":
            errormsg = plugintools.message("pascu","Por favor, introduzca el canal a buscar")
            return errormsg
...
def contextMenu(params):
    ...
    if len_canales == 1:
        ...
        if len_canales == 2:
            ...
            selector = dialog.select('pascu', [tv_a, tv_b])
            ...
            selector = dialog.select('pascu', [tv_a, tv_b, tv_c])
            ...
            selector = dialog.select('pascu', [tv_a, tv_b, tv_c, tv_d])
            ...
            selector = dialog.select('pascu', [tv_a, tv_b, tv_c, tv_d, tv_e])
            ...
            selector = dialog.select('pascu', [tv_a , tv_b, tv_c, tv_d, tv_e, tv_f])
            ...
            selector = dialog.select('pascu', [tv_a , tv_b, tv_c, tv_d, tv_e, tv_f, tv_g])
...
def xml_skin():
    ...
    if xmlmaster == 'true':
        if SelectXMLmenu == '0':  # Default skin
            mastermenu = 'http://pascu120.tk/pascu.xml'
            ...
            if id_pastebin == "":
                ...
                mastermenu = 'hhttp://pascu120.tk/pascu.xml'
            else:
