...
trunkfolder = "http://proyectoluzdigital.info/tvguia/download/MODPLDACE"
...
osx_i386_acestream = trunkfolder + "/Modules/MacOsx/acestream_osx64.tar.gz"
osx_x64_sopcast = trunkfolder + "/Modules/MacOsx/x86_64/sopcast_osx64.tar.gz"
osx_x64_acestream = trunkfolder + "/Modules/MacOsx/acestream_osx64.tar.gz"
#Windows Files
	...
	if xbmc.getCondVisibility('system.platform.linux') and not xbmc.getCondVisibility('system.platform.Android'):
		...
		if found == True:
			...
			if os.path.exists(xbmc_data_path) and uid == os.stat(xbmc_data_path).st_uid:
				android_binary_dir = os.path.join(xbmc_data_path, "files", "PLD-Plexus")
		...
		if available == True:			
			...
			if not xbmcvfs.exists(os.path.join('/Applications','Ace Stream.app')):
				...
				if xbmcvfs.exists(MAC_KIT):
					xbmc.sleep(1000)
					cmd = 'unzip /Applications/acestream_osx64.tar.gz'
